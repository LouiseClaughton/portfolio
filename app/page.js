        ...
        {page.fields.about &&
          <div className="px-12 lg:px-24 py-24 md:py-32 lg:py-42">
            <h2 className="font-bold text-3xl md:text-4xl leading-[40px] md:leading-[60px]">
              Hello, I’m Louise, a <span className="text-[#8BD2FF]">web designer and developer</span> passionate about the <span className="text-[#FF71D0]">weird and wonderful</span>. 
            </h2>
      ...
      <div id="projects" className="bg-[#1E1E1E] relative">
        {page.fields.featuredProjects &&
          <div className="px-12 lg:px-24 py-24 md:py-32 lg:py-42 flex flex-col gap-12">
            <div className="flex justify-between flex-col items-center gap-6 lg:flex-row">
              <h2 className="text-3xl md:text-4xl text-white">Things I've <span className="text-[#FF71D0]">made</span></h2>
              <a href="/projects">
                <button 
                    className="caption text-lg z-50 rounded-3xl py-2 px-4 border-2 border-white hover:bg-white text-white hover:text-black hover:cursor-pointer transition-colors"
                >
                    View more
                </button>
              </a>
            ...
            <div className="border-b border-[#555] w-full pb-6">
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {page.fields.highSkills.map((skill) => (
                  <span key={skill.fields.title} className="px-4 py-1 bg-[#FF6B9D22] text-[#FF6B9D] border border-[#FF6B9D44] rounded-full text-sm">
                    {skill.fields.title}
                  </span>
                ))}
              </div>
            ...
            <div className="border-b border-[#555] w-full pb-6">
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {page.fields.mediumSkills.map((skill) => (
                  <span key={skill.fields.title} className="px-4 py-1 bg-[#7EC8E322] text-[#7EC8E3] border border-[#7EC8E344] rounded-full text-sm">
                    {skill.fields.title}
                  </span>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {page.fields.lowSkills.map((skill) => (
                  <span key={skill.fields.title} className="px-4 py-1 bg-[#FFFFFF11] text-[#888] border border-[#555] rounded-full text-sm">
                    {skill.fields.title}
                  </span>
                ))}
              </div>
      ...
      <div id="contact-cta" className="bg-[#FF71D0]">
        <div className="px-12 lg:px-24 py-24 md:py-32 lg:py-42 flex flex-col items-center gap-12 relative">
          <div className="text-center text-white">
            <h2 className="text-center text-4xl text-white mb-4">Ready to get started?</h2>
            <p className="caption text-white text-2xl">Let's build something brilliant together.</p>
          </div>
          <a href="/contact">
            <button 
                className="caption text-lg z-50 rounded-3xl py-2 px-4 border-2 border-white hover:bg-white text-white hover:text-black hover:cursor-pointer transition-colors"
